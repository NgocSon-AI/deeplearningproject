...
class ModelTrainer:
    ...
    def train(self, optimizer: Optimizer) -> None:
        ...
        output: loss, batch_id, accuracy
        """
        logging.info("Entered the train method of Model trainer class")
        try:
            self.model.train()
            p_bar = tqdm(self.data_transformation_artifact.transformed_train_object)
            correct: int = 0
            processed = 0
            for batch_idx, (data, target) in enumerate(p_bar):
                ...
                pred = y_pred.torch.argmax(dim=1, keepdim=True)
